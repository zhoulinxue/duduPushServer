...
public class NSConfig {
    ...
    public NSConfig(String configJson) {
	super();
	// TODO Auto-generated constructor stub
	config = JSONObject.parseObject(configJson.trim(), NSConfig.class);
	builder = creatDefaultBuilder();
	config.setBuilder(builder);
    }
    ...
    public static NSConfig creatDefault() {
	    try {
		String cfg = FileUtil.BufferedReader("config.json");
		// System.out.println(cfg.trim());
		config = new NSConfig(cfg);
	    } catch (Exception e) {
		e.printStackTrace();
		Log.error("读配置文件config.json出错，方法：getServerIP|" + e.getMessage());
	    }
	return config;
    }
	...
	builder.setAppHandler(new AppHandlerServer()).setNodeHandler(new NodeHandlerServer()).setShorHandler(new ShorthandlerServer());
	return builder;
    }
	...
	return "NSConfig [redisip=" + redisip + ", redispwd=" + redispwd + ", jdbcurl=" + jdbcurl + ", jdbcuser="
		+ jdbcuser + ", jdbcpwd=" + jdbcpwd + ", serverip=" + serverip + ", nodeport=" + nodeport + ", appport="
		+ appport + ", nsip=" + nsip + ", nsport=" + nsport + ", listeningport=" + listeningport + ", userid="
		+ userid + ", heartbeat=" + heartbeat + ", redisport=" + redisport + "]";
    }
