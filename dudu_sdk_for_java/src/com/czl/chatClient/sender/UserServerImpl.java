...
public class UserServerImpl extends BaseMessageServiceImpl implements UserServer {
	...
	@Override
	public void CommadFilter(Channel channel, NettyMessage message, JsonParser parser) throws Exception {
		// TODO Auto-generated method stub
		String data = new String(message.getContent(), "UTF-8");
		String[] splits = data.split("\\|");
		switch (AppServerType.ofCommand(message.getHeader())) {
		case PU:
			List<Pushmessage> msgs = (List<Pushmessage>) parser.parseArray(splits[0], Pushmessage.class);
			List<String> ids = new ArrayList<>();
			for (Pushmessage msg : msgs) {
				ids.add(msg.getDataid());
			}
			NettyMessage prmsg = buildMessage(AppServerType.PR);
			prmsg.setContent(getContentByte(new JsonString(parser.toJSONString(ids))));
			sendMessage(prmsg, channel);
			break;
			case IM:
				Pushmessage pushmessage= (Pushmessage) parser.parseObject(splits[2],Pushmessage.class);
				NettyMessage ismsg=buildMessage(AppServerType.IS);
				ismsg.setContent(getContentByte(new JsonString(pushmessage.getDataid()+seporate()+splits[1])));
		...
		case SM:
			...
			JsonString smjsonString = parser.ObjectToJsonString(smresponbean);
			smjsonString.getBuilder().append(Constants.SEPORATE).append(message.getStringFromUerId());
	...
	public NettyMessage enterChannel(Channel channel, DuduUser user, Groupbean info, JsonParser parser,
		...
		jsonString.getBuilder().append(seporate()).append(parser.toJSONString(info));
	...
	@Override
	public NettyMessage callUser(Channel channel, DuduUser myUser, DuduUser info, JsonParser parser,
			SendMessageLisenter lisenter) throws UnsupportedEncodingException {
		// TODO Auto-generated method stub
		NettyMessage message = buildMessage(AppServerType.FS);
		JsonString JsonStr = new JsonString();
		JsonStr.getBuilder().append(parser.toJSONString(myUser)).append(seporate()).append(info.getUserid());
	...
	public void refuseCall(Channel channel, DuduUser myUser, DuduUser remoteUser, JsonParser parser,
		...
		JsonString jso = new JsonString();
		jso.getBuilder().append(parser.toJSONString(myUser)).append(seporate()).append(remoteUser.getUserid());
	...
	public void huangUp(Channel channel, DuduUser myUser, DuduUser remoteUser, JsonParser parser,
		...
		joStr.getBuilder().append(parser.toJSONString(myUser)).append(seporate()).append(remoteUser.getUserid());
        message.setContent(getContentByte(joStr));
        sendMessage(message,channel);
	}

	@Override
	public void sendAudoiByte(AppServerType type,Channel channel, byte[] bytes, SendMessageLisenter lisenter)
			throws UnsupportedEncodingException {
		// TODO Auto-generated method stub
	...
	public void sendFreedomMessage(Channel channel, String jsonObject, DuduUser touser, JsonParser parser,
		...
		message.setContent(getContentByte(new JsonString(parser.toJSONString(touser)+seporate()+jsonObject)));
		sendMessage(message, channel, lisenter);
	}
	...
	public void sendET(Channel channel, String time,String id, DuduUser myUser, JsonParser parser,
		...
		JsonString jso = new JsonString();
		jso.getBuilder().append(parser.toJSONString(bean)).append(seporate()).append(message.getStringMessageId());
		message.setContent(getContentByte(jso));
		sendMessage(message, channel, lisenter);
	}

	@Override
	public void sendGT(Channel channel, String time,String id, DuduUser myUser, JsonParser parser,
			SendMessageLisenter lisenter) throws UnsupportedEncodingException {
		// TODO Auto-generated method stub
		...
		JsonString jso = new JsonString();
		jso.getBuilder().append(parser.toJSONString(bean)).append(seporate()).append(id);
		message.setContent(getContentByte(jso));
		sendMessage(message, channel, lisenter);
	}
	...
	public void checkUserIsOnLine(Channel channel, DuduUser user, DuduUser frienduser, JsonParser parser,
		...
		JsonString jso = new JsonString();
		jso.getBuilder().append(parser.toJSONString(user)).append(seporate()).append(frienduser.getUserid());
		message.setContent(getContentByte(jso));
		sendMessage(message, channel, lisenter);
	}
	...
	public void cancelRequst(Channel channel, DuduUser user, DuduUser frienduser, JsonParser parser,
		...
		fejso.getBuilder().append(parser.toJSONString(user)).append(seporate()).append(frienduser.getUserid());
