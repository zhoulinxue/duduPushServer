...
public final class DuduSDK extends BaseMessageServiceImpl implements Runnable, onConnetCallBack {
	// 链接对象
	...
	public void run() {
		...
		try {
			...
			Log.e("Dudu_SDK", "getServer ...failed ...user  default server... ");
		}
	}
	...
	private void connectMS(String ip, int port) {
		...
		b.group(group).channel(NioSocketChannel.class).option(ChannelOption.TCP_NODELAY, true)
				.handler(new MSChannelInitializer(selfcallback, DuduSDK.this, paser));
		// 发起异步连接操作
		try {
			ChannelFuture future = b.connect(ip, port).channel().closeFuture().await();
			if (!future.isSuccess()) {
				sleepTime = 20 * 1000;
				reconnect();
			}
	...
	private void connectNS(String ip, int port) {
		...
		Bootstrap b = new Bootstrap();
		b.group(group).channel(NioSocketChannel.class).option(ChannelOption.TCP_NODELAY, true)
				.handler(new NSChannelInitializer(selfcallback, DuduSDK.this, paser));
		try {
			Log.e("", "NS=:" + ip + ":" + port);
			ChannelFuture future = b.connect(new InetSocketAddress(ip, port)).await();
			if (future.isSuccess()) {
	...
	protected void sendMessage(Channel channel, final NettyMessage message, final SendMessageLisenter listener) {
		// TODO Auto-generated method stub
		if (channel != null) {
	...
	public void onConnectSucess(Channel channel) {
		...
		if (user == null) {
			throw new IllegalStateException("did not  login.... call DuduClient.login(DuduUser user)....first");
		}
		...
		if (currentMsg != null) {
			...
			if (AppServerType.GB.toString().equals(header) && obg.length == 4) {
				try {
					Groupbean info = (Groupbean) paser.parseObject(obg[obg.length - 3], Groupbean.class);
...
//			} 
	...
	public void onConnectFailed() {
		// TODO Auto-generated method stub
		Log.e("Dudu_SDK", "获取NS 业务服务器失败...onConnectFailed ... reconnect()...5 seconds later ...");
		reconnect();
		if (connectCallBack != null)
	...
	public void disconnect(Channel channel) {
		// TODO Auto-generated method stub
		Log.e("Dudu_SDK", "disconnect   " + ((user == null) ? "true" : user.getUserid()));
		close(false);
		isConnected = false;
		reconnect();
		Log.e("Dudu_SDK", "connectCallBack==null   " + (connectCallBack == null));
		if (connectCallBack != null)
	...
	private RecivMessageCallBack selfcallback = new RecivMessageCallBack() {
		...
		@Override
		public void onRecivMessage(Channel channel, NettyMessage message, String tag, JsonParser parser) {
			// TODO Auto-generated method stub
			if (client == null) {
				throwUnInit();
				return;
		...
		private boolean messageFilter(NettyMessage message) {
			...
			try {
				...
				if (splits.length == 4) {
					...
					close(false);
					currentMsg = message;
					Log.e("", "切换服务器"+splits[splits.length - 2]+""+Integer.valueOf(splits[splits.length - 1]));
					Thread.sleep(500);
					connect(splits[splits.length - 2], Integer.valueOf(splits[splits.length - 1]));
					return false;
				} else if (AppServerType.EX_TYPE.toString().equals(message.getHeader()) && "100".equals(splits[0])) {
					user = null;
					setCallingUser(null);
					isConnected = false;
					isReconnect = false;
					close(false);
				} else if (AppServerType.EX_TYPE.toString().equals(message.getHeader()) && "0".equals(splits[0])) {
					sleepTime = 10 * 1000;
					isLooping = false;
					reconnect();
				} else if (AppServerType.AB.toString().equals(message.getHeader())) {
					...
					return false;
				}else if(AppServerType.RS.toString().equals(message.getHeader())){
					Responbean bean=(Responbean) paser.parseObject(splits[0],Responbean.class);
				...
				// 处理特殊消息
				ServerFacoty.getInstance().getUserserver().CommadFilter(mChannel, message, paser);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		try {
			...
			case FN:
				if (!isBusy()) {
					...
					DuduUser user = (DuduUser) paser.jsonStringToObJect(fnStr, DuduUser.class);
					Responbean responbean = new Responbean();
					responbean.setHeader(AppServerType.FS.getHeaderString());
					responbean.setResponeId(message.getStringMessageId());
					JsonString jsonString = paser.ObjectToJsonString(responbean);
					jsonString.getBuilder().append(Constants.SEPORATE).append(user.getUserid());
					responeRs(mChannel, message.getHeader(), jsonString);
				}
				break;
			case FA:
				setCallingUser(null);
				friendUser = (DuduUser) paser.parseObject(splits[0], DuduUser.class);
				changeChattingModel(true, false);
				break;
			case AL:
				ChannalActiveusers activeusers = (ChannalActiveusers) paser.parseObject(splits[0],
				...
				if (activeusers != null) {
					...
					if (!StringUtils.isEmpty(activeusers.getChannelNum())) {
						groupbean.setChannelNum(Integer.valueOf(activeusers.getChannelNum()));
					}
			...
			case ED:
				...
				break;
			case CG:
				groupbean = (Groupbean) paser.parseObject(splits[0], Groupbean.class);
				...
				break;
			case GC:
				Groupbean cggroupbean= (Groupbean) paser.parseObject(splits[0],Groupbean.class);
			...
			case CA:
				...
				List<String> cdchannemlIds = (List<String>) paser.parseArray(splits[0], String.class);
				if (cdchannemlIds == null || cdchannemlIds.size() == 0
						|| (groupbean != null && !cdchannemlIds.contains(groupbean.getGroupId()))) {
					if(isGroupChatting()){
					    changeChattingModel(false, false);
					}
				}
			...
			case NT:
				if (groupbean != null && paser != null) {
					DuduPosition feuser = (DuduPosition) paser.parseObject(splits[0], DuduPosition.class);
					groupbean.getActiveUsers().add(feuser);
				}
				break;
			case EG:
				if (groupbean != null && paser != null) {
					DuduPosition eguser = (DuduPosition) paser.parseObject(splits[0], DuduPosition.class);
	...
	protected void changeChattingModel(boolean isChatting, boolean isGroupChatting) {
		Log.e("", "是否在对讲：——" + isChatting + "是否是群聊：——" + isGroupChatting);
		this.isChatting = isChatting;
		this.isGroupChatting = isGroupChatting;
	}

	protected void dosomething(Channel channel, NettyMessage message, JsonParser parser) {
		// TODO Auto-generated method stub
		Log.e("", "界面登陆监听==null:" + (connectCallBack == null));
		if ((callBackMap == null ? 0 : callBackMap.entrySet().size()) == 0 && connectCallBack != null) {
			connectCallBack.addLisenter(client);
		}
		Log.e("", "监听器的数量=" + (callBackMap == null ? 0 : callBackMap.entrySet().size()));
		for (Map.Entry<String, RecivMessageCallBack> entry : callBackMap.entrySet()) {
			RecivMessageCallBack callBack = entry.getValue();
			if (callBack != null && AppServerType.contains(message.getHeader(), callBack)) {
				callBack.onRecivMessage(channel, message, entry.getKey(), parser);
			}
		}
